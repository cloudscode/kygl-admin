      ...
      <tree-table :data="list" :eval-func="func" :eval-args="args" :columns="columns" :expand-all="expandAll" border>
        <!-- <el-table-column :label="$t('menu.icon')" align="center">
          <template slot-scope="scope">
            <svg-icon v-if="scope.row.name" :icon-class="scope.row.icon"/>
          </template>
        </el-table-column>
...
import { fetchDictionaryItemList } from '@/api/dictionary/dictionary'
export default {
  name: 'Dictionaries',
  ...
  methods: {
    ...
    getList() {
      this.listLoading = true
      // fetchDictionaryItemList({ q: 'd=' + this.domain_id }).then(res => {
      fetchDictionaryItemList({}).then(res => {
        console.log('***************************')
